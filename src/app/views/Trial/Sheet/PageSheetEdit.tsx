...
export function PageSheetEdit() {
    ...
    useEffectOnce(() => {
        (async function () {
            let [sheet, { ret: details }] = await Promise.all([
                JsTicket.idObj(sheetId),
                JsTicket.GetDetails.query({ id: sheetId }),
            ]);
            setSheet(sheet);
    ...
    async function onSubmit() {
        setVisible(false);
        await JsTicket.BookSheetPurchase.submit({ id: sheetId });
        let { pageCache } = uqApp;
        ...
        await openModal(<Page header="提交成功" back="none">
            <div className="p-3">
                采购单 <b>{sheet.no}</b> 已提交
            </div>
            <div className="border-top p-3">
                <button className="btn btn-outline-primary" onClick={closeModal}>返回</button>
                <button className="ms-3 btn btn-outline-secondary" onClick={addDetailOnOk}>新建订单</button>
            </div>
        </Page>);
        navigate(-1);
    }
    ...
    async function onItemSelect(item: any) {
        let itemId = item.id;
        let ret = await openModal(<PageSheetDetail item={item} value={undefined} />);
        let { quantity } = ret;
        if (Number.isNaN(quantity) === false) {
            let id = await JsTicket.ActID({
                ID: JsTicket.Detail,
                value: {
                    sheet: sheetId,
                    item: itemId,
    ...
    async function onEditDetail(detail: Detail) {
        ...
        let { quantity } = await openModal(<PageSheetDetail item={item} value={value} />);
        await JsTicket.ActID({
            ID: JsTicket.Detail,
            value: {
                id,
                sheet: sheetId,
    ...
    return <Page header="单据">
        <div className="py-2 tonwa-bg-gray-3">
            ...
            <div className="px-3 py-2">
                <b>往来单位：</b><IDView id={sheet.vendor} uq={JsTicket} Template={ViewVendor} />
            </div>
        </div>
...
function PageSheetDetail({ item, value }: { item: any, value: number }) {
